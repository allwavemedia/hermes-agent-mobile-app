...
| `jose` | 6.2.4 | MIT | Standards-based JWS/JWK/JWT |
...
Python already directly pins `cryptography==46.0.7` and `websockets==15.0.1`, and already includes `PyJWT[crypto]`; reuse them. `pywin32` is already a Windows dependency. New Python `keyring>=25.7,<26` is the only planned host runtime addition; if security/platform testing favors direct `win32crypt` plus macOS Security APIs, ADR-003 must be amended and `keyring` removed.
...
| Python 3.14.6 / uv 0.11.26 | `uv` ready; installed Python unsupported for Hermes |
| JDK/Android Studio/SDK/Gradle/ADB/signing tools | Missing |
